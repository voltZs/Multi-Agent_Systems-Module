	...
	private class OfferListener extends CyclicBehaviour{
		...
		public void action() {
			...
			if (msg != null) {
				...
				String[] auctionTuple = msg.getContent().split(",");
				String auctionItem = auctionTuple[0];
				int auctionPrice = Integer.valueOf(auctionTuple[1]);
				
				System.out.println(getAID().getName() + " received auction offer for "+ auctionItem + " at price " + auctionPrice);
				ACLMessage reply = msg.createReply();
				
				Integer offerAmount = (Integer) toBuyList.get(auctionItem);
				if((offerAmount != null) && (auctionPrice <= offerAmount.intValue())) {
//					I have this item in my list - create proposal
					int newBidPrice = (int) Math.ceil(auctionPrice + (auctionPrice *(Math.random()/2)));
					if(newBidPrice < offerAmount.intValue()){
						offerAmount = (Integer) newBidPrice;
					}
